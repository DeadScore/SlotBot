...
async def event_create(
    ...
    # IMPORTANT: Discord interactions must be acknowledged within ~3 seconds.
    # Creating the post + reactions can take longer, so we defer immediately.
    try:
        await interaction.response.defer(ephemeral=True, thinking=True)
    except Exception:
        # If it was already acknowledged (rare), we'll just use followups.
        pass
    ...
    # Acknowledge (we already deferred), but keep the UX nice.
    try:
