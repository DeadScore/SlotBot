...
intents.message_content = True  # not needed for slash + reactions

bot = commands.Bot(command_prefix="!", intents=intents)

# discord.py 2.x: Sync Commands einmalig im setup_hook (nicht in on_ready), sonst wird es bei Reconnect spammy.
@bot.event
async def setup_hook():
    try:
        ...
        if gid:
            ...
            print(f"✅ Slash Commands synced (Guild {gid})")
        else:
...
if __name__ == "__main__":
    print("🚀 Starte SlotBot + Flask (Web Service) ...")
    if not DISCORD_TOKEN:
        raise RuntimeError("DISCORD_TOKEN ist nicht gesetzt (Render → Environment Variables).")

    def run_flask():
        port = int(os.environ.get("PORT", "10000"))
        # Render erwartet, dass dieser Port geöffnet ist.
        flask_app.run(host="0.0.0.0", port=port)

    threading.Thread(target=run_flask, daemon=True).start()

    # Discord-Bot startet blockierend; Auto-Reconnect ist bereits in discord.py drin.
